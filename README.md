...
> **v6.12.91** · code **v6.12.92** tagué, registry/images LIVE encore **v6.12.91** — module registry — bootstrap AWS Swarm + substrat AISIA
...
| Catalogue modèles | **867** modèles catalogue · **123** locaux déclarés · **58** locaux enabled |
| Stateless | Qdrant + audit AI Act + multi-tenant |
| SaaS opaque | Déployable Swarm/K8s — runtime **v6.12.91** · code **v6.12.92** |
...
| `image_tag` | Tag d'image AISIA à déployer | `string` | `"v6.12.92"` | non |
...
| `substrate` | `string` | `"swarm"` | Substrat cible. Ce module provisionne le substrat 'swarm'. |
...
| `image_tag` | `string` | `"v6.12.92"` | Tag d'image AISIA à déployer (ex. v6.12.92). |
...
| `region` | `string` | `"eu-west-3"` | Région AWS (eu-west-3 = Paris pour conformité RGPD). |
| `env` | `string` | `"prod"` | Environnement (prod | staging | dev). Utilisé pour le tagging AWS. |
| `cluster_name` | `string` | `"aisia-swarm"` | Nom logique du cluster Swarm (préfixe des ressources AWS). |
| `vpc_cidr` | `string` | `"10.40.0.0/16"` | CIDR du VPC (RFC1918). |
| `subnet_cidr` | `string` | `"10.40.1.0/24"` | CIDR du subnet public hébergeant les nœuds Swarm. |
...
| Name | Description |
|------|-------------|
| `region` | Région AWS du déploiement. |
| `node_count` | Nombre de workers provisionnés (hors manager). |
| `manager_ip` | IP publique du manager Swarm. |
| `manager_private_ip` | IP privée du manager (advertise-addr Swarm + cible des join). |
| `worker_ips` | IPs publiques des workers Swarm. |
| `vpc_id` | ID du VPC AISIA Swarm. |
| `subnet_id` | ID du subnet public hébergeant les nœuds. |
| `security_group_id` | ID du Security Group Swarm. |
| `swarm_join_token_path` | — |
| `swarm_join_command` | Gabarit de commande join (le token réel est sur le manager). |
| `endpoints` | Endpoints applicatifs exposés par le manager (Traefik HTTP/HTTPS). |
| `next_steps` | Étapes post-apply : join workers + déploiement stack AISIA. |
<!-- END_TF_DOCS -->
...
- **Version module / code** : **v6.12.92**
- **PROD LIVE documentaire** : **v6.12.91** (runtime cluster ; distinct du tag module)

<!-- TF-REGISTRY-STATUS -->
## Statut publication registry (honnête)

> Mesuré à la régénération docs · **version code TF** **v6.12.92** (`VERSION` modules + provider) · PROD LIVE documentaire **v6.12.91**.

| Artefact | Repo | Public registry.terraform.io |
|----------|------|------------------------------|
| Provider `aisia-foundation/aisia` | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-aisia-cluster` (`cluster/aisia`) | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-aisia-swarm` (`swarm/aisia`) | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-aws-aisia` (`aisia/aws`) | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-azure-aisia` (`aisia/azure`) | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-google-aisia` (`aisia/google`) | `6.12.92` | **absent public** ⚠️ |
| Module `terraform-ovh-aisia` (`aisia/ovh`) | `6.12.92` | **6.12.91** ❌ écart |
| Module `terraform-scaleway-aisia` (`aisia/scaleway`) | `6.12.92` | **6.12.91** ❌ écart |
